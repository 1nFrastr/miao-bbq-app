...
import { Post, SortType, UserLocation } from '../../types'
import './index.scss'
...
export default function CommunityPage() {
  ...
  const loadPosts = () => {
    ...
    if (savedPosts.length === 0) {
      const { DEMO_POSTS } = require('../../utils/demoData')
      StorageService.set(STORAGE_KEYS.COMMUNITY_POSTS, DEMO_POSTS)
      setPosts(DEMO_POSTS)
    } else {
      setPosts(savedPosts)
    }
  ...
  const handleImageChange = (files: any[]) => {
    setUploadedImages(files.slice(0, 3))
  }
  ...
  const publishPost = () => {
    ...
    const newPost: Post = {
      ...
      images: uploadedImages.map(img => img.url || img),
